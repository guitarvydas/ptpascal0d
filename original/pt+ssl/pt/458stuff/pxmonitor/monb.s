| PX SUN Monitor (Part B)  Ver 1.03, 
...
| PX globals defined in C
	.globl	_PXConcatenate
	.globl	_PXSubstring
	.globl	_PXReadString
	.globl	_PXWriteString
...
| global trap names used in PX program
...
t101:	| pxFetchLiteralString (literalAddress : address) : string
...
t102:	| pxFetchString (stringAddress : address) : string
...
t103:	| pxConcatenate (s1, s2 : string) : string
...
t104:	| pxSubstring (s : string; i1,i2 : integer) : string
...
t105:	| pxLength (s : string) : integer
	| 	replaces two string values on stack with their concatenation
	movl	sp@+, return	| return address
...
t106:	| pxStringEqual (s1, s2 : string) : boolean
...
t107:	| pxReadString (stringAddress : address; fd : integer)
	movl	sp@+, return
	jbsr	_PXReadString
	addql	#8, sp		| pop args
	movl	return, sp@-
...
t108:	| pxWriteString (s : string; w : integer; fd : integer)
	movl	sp@+, return
	pea	sp@(8)		| addr(s)
	jbsr	_PXWriteString
